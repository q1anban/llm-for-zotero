...
function buildAssistantNoteHtml(contentText: string, modelName: string): string {
  ...
  return `<div>${responseHtml}</div><p>Generated by ${escapeNoteHtml(source)} model, written by Zotero-LLM plugin</p>`;
}
...
async function createNoteFromAssistantText(
  ...
  note.setNote(buildAssistantNoteHtml(contentText, modelName));
  await note.saveTx();
}
  ...
  if (responseMenu && responseMenuCopyBtn && responseMenuNoteBtn) {
    if (!responseMenu.dataset.listenerAttached) {
      ...
      responseMenuCopyBtn.addEventListener("click", async (e: Event) => {
        ...
        await copyTextToClipboard(body, responseMenuTarget.contentText || "");
      ...
      responseMenuNoteBtn.addEventListener("click", async (e: Event) => {
        ...
        try {
          await createNoteFromAssistantText(
            ...
            responseMenuTarget.contentText,
    ...
    if (isUser) {
      ...
      if (hasAnswerText) {
        ...
        bubble.addEventListener("contextmenu", (e: Event) => {
          ...
          const contentText = selectedText || fallbackText;
